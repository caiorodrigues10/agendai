...
interface FinancialDashboardProps {
  queueHistory: QueueItem[]; // All items including completed/cancelled
  services: Service[];
...
export const FinancialDashboard: React.FC<FinancialDashboardProps> = ({ 
  ...
  const [viewMode, setViewMode] = useState<'personal' | 'shop'>(currentUser.role === 'admin' ? 'shop' : 'personal');
  ...
  const filteredData = useMemo(() => {
    ...
    return queueHistory.filter(item => {
      ...
      // 2. Filter by User (Shop vs Personal)
  ...
  const stats = useMemo(() => {
    ...
    const maxDayCount = Math.max(...daysCount) || 1; // Avoid divide by zero
                ...
                <table className="w-full text-left border-collapse">
                    <thead className="bg-neutral-950 text-neutral-500 text-[10px] uppercase tracking-wider sticky top-0">
                        <tr>
                            ...
                            {currentUser.role === 'admin' && <th className="p-3 font-medium text-center">Ações</th>}
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-neutral-800">
                        {filteredData.map((item) => (
                            <tr key={item.id} className="hover:bg-neutral-800/50 transition-colors text-xs">
                                <td className="p-3 text-neutral-400">
                                    <div className="flex flex-col">
                                        <span className="text-neutral-300 font-medium">
                                            {new Date(item.completedAt!).toLocaleDateString('pt-BR', {day:'2-digit', month:'2-digit'})}
                                        </span>
                                ...
                                <td className="p-3 font-medium text-white">{item.customerName}</td>
                                <td className="p-3 text-cyan-300">{getServiceName(item.serviceId)}</td>
                                {viewMode === 'shop' && (
                                    <td className="p-3 text-right text-neutral-400">{getStaffName(item.completedBy)}</td>
                                )}
