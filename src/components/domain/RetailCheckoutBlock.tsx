...
export const RetailCheckoutBlock: React.FC<Props> = ({ canOverridePrice, onChange }) => {
  ...
  useEffect(() => {
    if (!open) return;
    const t = setTimeout(() => {
      productsApi.listProducts({ search, forSale: 'true', active: 'true', limit: 30 }).then(setProducts).catch(err => setError(getErrorMessage(err, 'Não foi possível buscar produtos.')));
    }, 200);
  ...
  }, [open, cart, method, total, canOverridePrice, onChange]);
      ...
      <div className="max-h-40 overflow-y-auto space-y-1">
        {products.map(product => (
          ...
          >
            ...
            <span className="text-text-muted">R$ {product.salePrice.toFixed(2)}</span>
          </button>
        ))}
      </div>
      {cart.map(item => (
