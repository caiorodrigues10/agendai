...
export const PublicLinkPanel: React.FC<PublicLinkPanelProps> = ({ barbershopId, operationMode }) => {
  ...
  return <section className="space-y-4">
    <header className="rounded-2xl border border-border bg-surface p-4 sm:p-5"><p className="text-[10px] font-bold uppercase tracking-[0.22em] text-text-muted">Links públicos</p><h3 className="mt-1 text-lg font-bold text-text-primary">Compartilhe o destino certo</h3><p className="mt-1 text-sm text-text-secondary">Cada link tem um QR Code próprio, pronto para imprimir ou divulgar.</p></header>
    <div className="grid gap-4 xl:grid-cols-3">{destinations.map(item => { const Icon = item.icon; const isCopied = copied === item.id; return <article key={item.id} className="rounded-2xl border border-border bg-surface p-4 shadow-lg">
      <div className="flex items-start gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-surface-2 text-text-secondary"><Icon size={19} /></span><div><h4 className="text-sm font-bold text-text-primary">{item.label}</h4><p className="mt-1 text-xs text-text-secondary">{item.description}</p></div></div>
