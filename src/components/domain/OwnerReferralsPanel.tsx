...
export const OwnerReferralsPanel: React.FC = () => {
	...
	const conversionRate =
		data.stats.total > 0
			? ((data.stats.converted / data.stats.total) * 100).toFixed(0)
			: '0'
		...
		<div className="space-y-5">
			...
			<div className="rounded-2xl border border-border bg-surface p-4 space-y-3">
				...
				<p className="text-[11px] text-text-muted">
					Código:{' '}
					<strong className="text-text-secondary">{data.code}</strong> · +
					{data.rewardDays} dias por conversão
				</p>
			</div>
			...
			<div className="rounded-2xl border border-border bg-surface overflow-hidden">
				...
				{data.referrals.length === 0 ? (
					<p className="p-4 text-sm text-text-muted">
						Nenhuma indicação ainda. Compartilhe seu link com outros donos de
						salão.
					</p>
				) : (
