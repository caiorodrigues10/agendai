...
const PainSolution = ({ pain, solution, delay }: { pain: string; solution: string; delay: number }) => {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!ref.current) return;
    gsap.fromTo(ref.current, { opacity: 0, x: -30 }, {
      opacity: 1, x: 0, duration: 0.6, delay,
      ...
      <div className="space-y-3">
        ...
        <div className="mt-4 p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider">Próximo aviso</span>
          </div>
          <div className="text-[10px] text-neutral-400">Lucas será notificado quando faltar 5 min</div>
        </div>
      </div>
    </PhoneMockup>
  );
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.fromTo('.hero-badge', { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6 })
        ...
        .fromTo(heroSubRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6 }, '-=0.4')
        .fromTo(heroCtaRef.current, { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6 }, '-=0.3');

      // Phone mockup parallax on hero
      gsap.fromTo('.hero-phone', { opacity: 0, y: 60, rotationY: -10 }, {
      ...
      gsap.utils.toArray<HTMLElement>('.feature-section').forEach((section) => {
        ...
        if (textEl) {
          gsap.fromTo(textEl, { opacity: 0, x: -50 }, {
            opacity: 1, x: 0, duration: 0.8, ease: 'power3.out',
            scrollTrigger: { trigger: section, start: 'top 75%' },
          });
        }
        if (visualEl) {
          gsap.fromTo(visualEl, { opacity: 0, x: 50, scale: 0.95 }, {
      ...
      // Staff cards stagger
      gsap.fromTo('.staff-card', { opacity: 0, y: 40, scale: 0.95 }, {
      ...
      gsap.utils.toArray<HTMLElement>('.section-title').forEach((el) => {
        gsap.fromTo(el, { opacity: 0, y: 40, clipPath: 'inset(100% 0 0 0)' }, {
      ...
      gsap.utils.toArray<HTMLElement>('.bullet-item').forEach((container) => {
        gsap.fromTo(container.children, { opacity: 0, x: -20 }, {
      ...
      gsap.utils.toArray<HTMLElement>('.metric-value').forEach((el) => {
        gsap.fromTo(el, { opacity: 0, scale: 0.5, y: 20 }, {
    ...
    <div ref={pageRef} className="min-h-screen bg-black text-neutral-100 selection:bg-emerald-500/30 font-sans overflow-x-hidden">
      ...
      <section className="relative pt-40 pb-20 md:pt-52 md:pb-28 px-6 overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16 relative z-10">
          <div className="flex-1 text-center lg:text-left">
            ...
            <h1 ref={heroTitleRef} className="text-5xl md:text-7xl font-black tracking-tighter mb-6 leading-[1.05] opacity-0">
              <span className="bg-gradient-to-b from-white via-white to-white/30 bg-clip-text text-transparent">Seu salão para de perder cliente por falta de controle.</span>
            </h1>
            <p ref={heroSubRef} className="text-lg md:text-xl text-neutral-400 max-w-xl font-light leading-relaxed mb-10 mx-auto lg:mx-0 opacity-0">
              Fila digital, agendamento 24/7, financeiro, equipe e WhatsApp — tudo num só lugar. Sem planilha. Sem caderno. Sem dor de cabeça.
            </p>
            <div ref={heroCtaRef} className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start opacity-0">
              <button onClick={() => navigate('/login')} className="bg-emerald-500 text-white px-8 py-4 rounded-2xl font-bold text-sm hover:bg-emerald-400 transition-all flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(16,185,129,0.3)]">
                Começar grátis
                <ArrowRight className="w-4 h-4" />
              </button>
              <button onClick={() => navigate('/queue')} className="bg-white/5 border border-white/10 text-white px-8 py-4 rounded-2xl font-bold text-sm hover:bg-white/10 transition-all flex items-center justify-center gap-2">
      ...
      <section className="py-16 px-6 relative z-10 border-y border-white/5 bg-neutral-950/50">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-400">O que muda</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-1">
            <PainSolution pain="Cliente fica parado na fila sem saber quanto falta" solution="Acompanha tempo real no celular" delay={0} />
            <PainSolution pain="Dupla marcação e horários conflitantes" solution="Agenda calcula tudo automaticamente" delay={0.05} />
            <PainSolution pain="Ligando para confirmar cada agendamento" solution="WhatsApp envia lembrete sozinho" delay={0.1} />
            <PainSolution pain="Fim do mês e não sabe quanto lucrou" solution="Dashboard com faturamento diário" delay={0.15} />
            <PainSolution pain="Crediário em caderno, sem controle" solution="Fiado digital com pagamentos parciais" delay={0.2} />
            <PainSolution pain="Funcionário sem horário definido" solution="Escalas, férias e folgas organizadas" delay={0.25} />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
          <div className="feature-text flex-1">
            ...
            <p className="section-subtitle text-neutral-400 text-lg leading-relaxed font-light mb-8 max-w-lg">
              De casa, do trabalho, do bar. Ele abre o link, escolhe o serviço e pronto — sabe exatamente quando chegar. Sem ligar. Sem perguntar. Sem ficar no escuro.
            </p>
      ...
      <section className="feature-section py-24 md:py-36 px-6 relative z-10 bg-neutral-950/50 border-y border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col-reverse lg:flex-row items-center gap-16">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['09:00', '10:30', '14:00'].map((time, i) => (
                    <div key={time} className={`p-2 rounded-lg text-center text-[10px] font-bold ${
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['15:30', '16:00', '17:30'].map((time) => (
                    <div key={time} className="p-2 rounded-lg text-center text-[10px] font-bold bg-white/5 text-neutral-400">
                      {time}
                    </div>
                  ))}
                </div>
          ...
          <div className="feature-text flex-1">
            ...
            <p className="section-subtitle text-neutral-400 text-lg leading-relaxed font-light mb-8 max-w-lg">
              3 da manhã, cliente marca horário. Sistema calcula disponibilidade, evita conflito e confirma no WhatsApp. Você só vê o agendamento pronto na manhã seguinte.
            </p>
            <div className="bg-neutral-900/60 rounded-2xl border border-white/5 p-6">
              <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider mb-4">O que acontece em 3 segundos</div>
              <div className="space-y-3">
                {[
      ...
      <section className="feature-section py-24 md:py-36 px-6 relative z-10">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
          <div className="feature-text flex-1">
            ...
            <p className="section-subtitle text-neutral-400 text-lg leading-relaxed font-light mb-8 max-w-lg">
              Sabe quanto cada profissional faturou. Qual serviço dá mais lucro. Quando o fiado está atrasado. Tudo em tempo real — sem esperar o fim do mês pra contar.
            </p>
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={item.label} className="bg-neutral-900/60 rounded-xl border border-white/5 p-4">
      ...
      <section className="py-24 md:py-36 px-6 relative z-10 bg-neutral-950/50 border-y border-white/5">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="section-subtitle text-neutral-400 text-lg leading-relaxed font-light max-w-xl mx-auto mb-12">
            Dono vê tudo. Funcionário vê só o seu. Horários, férias, produtividade — tudo organizado sem WhatsApp de grupo.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              { name: 'Carlos', role: 'Barbeiro', today: '8 atendimentos', revenue: 'R$ 480', avatar: 'C' },
              { name: 'Ana', role: 'Cabeleireira', today: '6 atendimentos', revenue: 'R$ 420', avatar: 'A' },
              { name: 'Pedro', role: 'Barbeiro', today: '10 atendimentos', revenue: 'R$ 560', avatar: 'P' },
            ].map((person) => (
              <div key={person.name} className="staff-card bg-neutral-900/60 rounded-2xl border border-white/5 p-6 text-left opacity-0">
      ...
      <section className="feature-section py-24 md:py-36 px-6 relative z-10">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16">
          <div className="feature-visual flex-1 flex justify-center phone-parallax">
            <PhoneMockup>
              <div className="space-y-3">
                <div className="text-[10px] font-bold text-emerald-400 uppercase tracking-wider mb-2">Mensagens automáticas</div>
                {[
                  { time: 'Ontem 18:00', msg: 'Olá Lucas! Seu agendamento é amanhã às 14:00 com Carlos. Confirma? ✅', from: 'bot', status: 'Entregue' },
                  { time: 'Ontem 18:02', msg: 'Confirmado! Obrigado.', from: 'user' },
                  { time: 'Hoje 12:00', msg: 'Oi Lucas! Falta 2h para seu horário. Te esperamos! 💈', from: 'bot', status: 'Entregue' },
                  { time: 'Hoje 13:55', msg: 'Lucas, sua vez! Pode ir pro salão. Carlos está esperando.', from: 'bot', status: 'Entregue' },
                ].map((msg, i) => (
                  <div
                    key={i}
          ...
          <div className="feature-text flex-1">
            ...
            <p className="section-subtitle text-neutral-400 text-lg leading-relaxed font-light mb-8 max-w-lg">
              O cliente recebe mensagem como se fosse do salão. Confirmação, lembrete, "sua vez" — tudo automático, tudo no WhatsApp que ele já usa. Sem app. Sem link estranho.
            </p>
            <div className="bg-neutral-900/60 rounded-2xl border border-white/5 p-6">
              <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-wider mb-4">Automatizações</div>
              <div className="space-y-3">
                {[
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-center gap-16 lg:flex-row-reverse">
          <div className="feature-text flex-1">
            ...
            <div className="rounded-2xl border border-white/5 bg-neutral-900/60 p-6">
              ...
              <div className="space-y-3">
                {[
                  { label: 'Nome, status e horário de hoje', detail: 'Aberto / fechado em tempo real' },
      ...
      <section className="py-24 md:py-36 px-6 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="section-subtitle text-neutral-400 text-lg leading-relaxed font-light max-w-xl mx-auto mb-12">
            Dados de cada barbearia completamente separados. Autenticação por papéis, senhas criptografadas, assinatura validada por CPF.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              <div key={item.title} className="security-card bg-neutral-900/60 rounded-2xl border border-white/5 p-6 opacity-0">
