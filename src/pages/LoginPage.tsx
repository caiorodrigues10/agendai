			...
			<div className="flex-1 flex flex-col justify-center gap-10 py-12">
				...
				>
					...
					<p className="mt-4 text-neutral-400 text-sm xl:text-base max-w-md leading-relaxed font-light">
						Fila digital, agenda, financeiro e clientes — tudo em um painel
						que trabalha por você, em tempo real.
					</p>
				</motion.div>
...
export const LoginPage: React.FC = () => {
	...
	useEffect(() => {
		...
		script.async = true
		script.defer = true
		script.onload = () => { if (window.google?.accounts?.id) setGoogleLoaded(true) }
	...
	useEffect(() => {
		...
		try {
			...
			google.accounts!.id!.initialize({
				...
				callback: ({ credential }: { credential?: string }) => { if (credential) handleGoogleCredential(credential) },
				auto_select: false,
	...
	const navigateAfterAuth = (opts?: { forceCheckout?: boolean }) => {
		...
		if (planId) {
			...
			navigate(
				`/checkout?planId=${encodeURIComponent(planId)}&billing=${billing}&setup=trial`,
			)
			return
		}
			...
			<div className="relative flex items-center justify-center p-4 sm:p-6 lg:p-8">
				...
				>
					<div
						...
						style={{
							background:
								'linear-gradient(to right, transparent, var(--ag-accent), transparent)',
						}}
					/>
