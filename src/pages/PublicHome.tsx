      ...
      return (
        <div className="min-h-screen bg-bg text-text-primary flex flex-col items-center justify-center gap-4 px-6 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-text-secondary mt-1">Não foi possível carregar os dados deste estabelecimento.</p>
          </div>
          <button
            type="button"
            onClick={() => navigate('/')}
    ...
    <div className="min-h-screen pb-20 bg-bg text-text-primary">
      ...
      <main className="max-w-md mx-auto px-4 pt-6">
        <div className="flex bg-surface p-1 rounded-xl mb-6 border border-border relative">
            ...
            <div className={`absolute top-1 bottom-1 w-[calc(33.3%-2px)] bg-surface-2 rounded-lg transition-all duration-300
          ...
          <>
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-6">
                  <div>
                     ...
                     <span className="text-xs text-text-muted uppercase tracking-widest font-semibold ml-1">Tempo Estimado</span>
                  </div>
                  ...
                  {aiInsight && (
                     <span className={`text-xs px-3 py-1 rounded-full font-bold uppercase border tracking-wide shadow-lg
                       ${!isOpen ? 'bg-surface-2 text-text-secondary border-border-strong' :
                        aiInsight.busyLevel === 'high' ? 'bg-danger/15 text-danger border-danger/30 shadow-danger/10' :
                         aiInsight.busyLevel === 'medium' ? 'bg-warning/15 text-warning border-warning/30 shadow-warning/10' : 'bg-success/15 text-success border-success/30 shadow-success/10'}
                     `}>
                       { !isOpen ? 'Fechado' :
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
               <div className="bg-surface p-4 rounded-xl border border-border text-center shadow-lg group hover:border-accent/30 transition-colors">
                  <span className="block text-3xl font-bold text-text-primary mb-1 group-hover:text-accent transition-colors">{peopleWaiting}</span>
                  <span className="text-[10px] text-text-muted uppercase tracking-widest font-bold">Na Fila</span>
               </div>
               <div className="bg-surface p-4 rounded-xl border border-border text-center shadow-lg group hover:border-success/30 transition-colors">
                  ...
                  <span className="text-[10px] text-text-muted uppercase tracking-widest font-bold">Em Atendimento</span>
               </div>
            </div>
              ...
              {activeQueue.length === 0 ? (
                <div className="text-center py-16 bg-surface/50 rounded-2xl border-2 border-dashed border-border/60 flex flex-col items-center justify-center">
                  ...
                  {!isUserInQueue && isOpen && (
                     <button onClick={() => setShowJoinForm(true)} className="text-accent text-sm font-bold mt-2 hover:underline">
                        Seja o primeiro!
                     </button>
                  )}
                </div>
